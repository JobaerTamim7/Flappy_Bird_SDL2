...
bool common::typer(SDL_Renderer* renderer,  const std::string text, Uint32 start_time)
{
	Uint32 ellapse = SDL_GetTicks() - start_time;
	int t_char = text.length();
	size_t out_char_num = ellapse / 50;
	...
	SDL_Texture* text_tex = common::load_font_texture("asset/font/roboto.ttf", out_str + "|", renderer, BLACK, 36);

	SDL_Rect dst_rect;
	dst_rect.x = 100;
	dst_rect.y = 100;
	dst_rect.w = (200 / t_char) * out_char_num + 15;
	dst_rect.h = 30;
